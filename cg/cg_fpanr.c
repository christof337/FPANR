...
/*void d_log(double_st * res, const double_st a) {
  ...
  res->_value = log(val);
  d_setPrec(res,p+int(log2(log(val))));
}*/
...
/*void d_cos(double_st * res, const double_st a) {
  ...
  double val = d_getVal(a,&p);
  res->_value = cos(val);
  d_setPrec(res, p+int(log2(cos(val)/(val*sin(val)))));
}*/

double myCos(const double a) {
  double_st res, val;
  val._value = a;
  d_cos(&res, val);
  return res._value;
...
/*void d_sin(double_st * res, const double_st a) {
  ...
  double val = d_getVal(a,&p);
  res->_value = sin(val);
  d_setPrec(res, p+int(log2(sin(val)/(val*cos(val)))));
}*/
...
*/
{
  const double r8_pi = dtfp(3.141592653589793);
  double r1;
  double r2;
  static int used = -1;
  double x;
  // 0 in FPANR = 0x0000000000000001 
  static double y = 0x0000000000000001;
...
*/
  if ( ( used % 2 )== 0 )
  {
    ...
    x = mySqrt ( dtfp(-2.0) * myLog ( r1 ) ) * myCos ( dtfp(2.0) * r8_pi * r2 );
    y = mySqrt ( dtfp(-2.0) * myLog ( r1 ) ) * mySin ( dtfp(2.0) * r8_pi * r2 );
  }
  else
  {
...
*/
{
  ...
  *seed = 16807 * ( *seed - k * 127773 ) - k * 2836;
  ...
  r = ( dtfp( ( double ) ( *seed ) ) ) * dtfp(4.656612875E-10);
...
*/
  ...
  for ( i = 0; i < n; i++ )
  {
    for ( j = 0; j < n; j++ )
    {
      ...
      for ( k = 0; k < n; k++ )
      {
        ah[i+j*n] = ah[i+j*n] - dtfp(2.0) * a[i+k*n] * v[k] * v[j] / v_normsq;
      }
    }
  }
/*
...
*/
{
  ...
  {
    j = 0;
    a[i+j*ndiag] = dtfp(2.0);
  }
  for ( i = 0; i < n - 1; i++ )
  {
    j = 1;
    a[i+j*ndiag] = dtfp(-1.0);
  }
...
*/
{
  ...
  for ( i = 0; i < n; i++ )
  {
    ...
    *seed = 16807 * ( *seed - k * 127773 ) - k * 2836;
    ...
    r[i] = dtfp(( double ) ( *seed )) * dtfp(4.656612875E-10);
  }
